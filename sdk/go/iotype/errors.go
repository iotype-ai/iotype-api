...
// Error is the base error returned by this package. Use errors.As to inspect
// the HTTP status.
type Error struct {
	Status  int
	Message string
...
type InsufficientTokensError struct{ *Error }
...
type RealtimeError struct{ *Error }
...
func errorForStatus(status int, raw []byte) error {
	...
	base := &Error{Status: status, Message: truncate(raw, 500)}
